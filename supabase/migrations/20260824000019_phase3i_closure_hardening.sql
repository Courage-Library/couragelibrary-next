-- ============================================================================
-- COURAGE LIBRARY — PHASE 3I: CLOSURE HARDENING & FINANCIAL REMEDIATION
-- Target Database: couragelibrary-next
-- ============================================================================

-- 1. Payment Order State Transition Validation Trigger
CREATE OR REPLACE FUNCTION public.fn_validate_payment_order_transition()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- CREATED  -> PENDING, CANCELLED, EXPIRED, PAID
    -- PENDING  -> PAID, FAILED, CANCELLED, EXPIRED
    -- PAID     -> REFUNDED
    -- FAILED   -> PENDING (retry attempt), CANCELLED
    -- EXPIRED  -> (terminal)
    -- CANCELLED-> (terminal)
    -- REFUNDED -> (terminal)

    IF OLD.status = 'PAID' AND NEW.status != 'REFUNDED' THEN
        RAISE EXCEPTION 'Illegal state transition: PAID order cannot transition to %', NEW.status;
    ELSIF OLD.status IN ('CANCELLED', 'EXPIRED') AND NEW.status != OLD.status THEN
        RAISE EXCEPTION 'Illegal state transition: % order cannot be updated to %', OLD.status, NEW.status;
    ELSIF OLD.status = 'REFUNDED' AND NEW.status != 'REFUNDED' THEN
        RAISE EXCEPTION 'Illegal state transition: REFUNDED order is terminal and cannot transition to %', NEW.status;
    END IF;
...
DROP TRIGGER IF EXISTS trg_validate_payment_order_transition ON public.payment_orders;
CREATE TRIGGER trg_validate_payment_order_transition
    BEFORE UPDATE OF status ON public.payment_orders
    FOR EACH ROW EXECUTE FUNCTION public.fn_validate_payment_order_transition();


-- 2. Hardened fn_create_payment_order with Active Reservation Cap Counting
...
BEGIN
    ...
    IF p_idempotency_key IS NOT NULL THEN
        SELECT id, status, total_payable_amount INTO v_order_id, v_existing_status, v_total_amount
    ...
    IF p_coupon_code IS NOT NULL AND length(trim(p_coupon_code)) > 0 THEN
        ...
        -- Hardened Global Usage Check: Count COMMITTED usage + ACTIVE RESERVATIONS (last 30 minutes)
        IF v_coupon.max_total_usage IS NOT NULL THEN
            ...
            IF (v_coupon.current_usage_count + v_active_reservations) >= v_coupon.max_total_usage THEN
                RETURN jsonb_build_object('success', false, 'error', 'Coupon usage limit has been reached');
            END IF;
        END IF;

        -- Validate per-user usage limit (COMMITTED + ACTIVE RESERVED)
        SELECT COUNT(*) INTO v_user_coupon_count
        FROM public.coupon_redemptions
        WHERE coupon_id = v_coupon.id
          AND user_id = v_user_id
          AND (status = 'COMMITTED' OR (status = 'RESERVED' AND reserved_at >= (now() - interval '30 minutes')));
...
-- 3. Hardened fn_verify_and_fulfill_payment with Subscription Continuous Extension
...
BEGIN
    ...
    -- Reject terminal or invalid non-payable states
    IF v_order.status IN ('CANCELLED', 'EXPIRED', 'REFUNDED') THEN
        RETURN jsonb_build_object('success', false, 'error', 'Cannot fulfill order in ' || v_order.status || ' state');
    END IF;
    ...
    -- 4. Issue Entitlement into user_entitlements with Continuous Subscription Extension
    IF v_order.order_type = 'SUBSCRIPTION_PURCHASE' THEN
        ...
        -- Check for existing active subscription with valid_until in the future
        SELECT valid_until INTO v_current_expiry
        ...
        IF v_current_expiry IS NOT NULL AND v_current_expiry > now() THEN
            v_expires_at := v_current_expiry + (v_plan.duration_days || ' days')::interval;
        ELSE
