...
export function AdaptiveAnalyticsView({ userEmail }: { userEmail: string }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 rounded-xl bg-white border border-slate-200/80 shadow-xs">
        ...
        <div className="flex items-center gap-2">
            ...
            {(["today", "7d", "30d", "90d", "all"] as const).map((t) => (
              <button
                ...
                className={`px-2.5 py-1 rounded-md capitalize font-medium transition ${
                  timeWindow === t ? "bg-blue-600 text-white font-semibold" : "text-slate-600 hover:text-slate-900"
                }`}
              >
                {t === "all" ? "All Time" : t}
              </button>
          ...
          <button
            ...
            disabled={isPending}
            className="p-2 rounded-lg bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 transition flex items-center justify-center"
      ...
      {errorMessage && (
        <div className="p-3 rounded-lg bg-rose-50 border border-rose-200 text-rose-800 text-xs font-medium flex items-center justify-between">
          ...
          <button onClick={() => setErrorMessage(null)} className="text-slate-400 hover:text-slate-900">✕</button>
        </div>
      )}
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
            <div className="p-3.5 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[11px] font-semibold text-slate-500 block uppercase tracking-wider">Total Attempts</span>
              <span className="text-xl font-bold text-slate-900 mt-1 block">{overview.totalAttempts}</span>
              <span className="text-[10px] text-slate-500 mt-0.5 block">{overview.inProgressAttempts} in progress</span>
            </div>
            <div className="p-3.5 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[11px] font-semibold text-slate-500 block uppercase tracking-wider">Completion Rate</span>
            ...
            <div className="p-3.5 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[11px] font-semibold text-slate-500 block uppercase tracking-wider">Avg Questions</span>
            ...
            <div className="p-3.5 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[11px] font-semibold text-slate-500 block uppercase tracking-wider">Avg Final θ</span>
            ...
            <div className="p-3.5 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[11px] font-semibold text-slate-500 block uppercase tracking-wider">Avg Final SE</span>
            ...
            <div className="p-3.5 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[11px] font-semibold text-slate-500 block uppercase tracking-wider">CAT Fallback Rate</span>
              ...
              {Object.keys(overview.selectionStrategyDistribution).length === 0 ? (
                ...
                <div className="space-y-2">
                  {Object.entries(overview.selectionStrategyDistribution).map(([strategy, count]) => {
                    return (
                      <div key={strategy} className="flex items-center justify-between p-2 rounded-lg bg-slate-50 border border-slate-200">
                        <span className="text-xs font-mono text-slate-300">{strategy}</span>
                        <Badge variant="outline" className="text-xs font-mono font-bold bg-slate-100 text-slate-700 border border-slate-200">
                          {count} decisions
                        </Badge>
                      </div>
                    );
      ...
      {subTab === "candidates" && candidates && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="p-3.5 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[11px] font-semibold text-slate-500 block uppercase">Total Candidates Tracked</span>
              <span className="text-xl font-bold text-slate-900 mt-1 block">{candidates.totalCandidates}</span>
            </div>
            <div className="p-3.5 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[11px] font-semibold text-slate-500 block uppercase">Active in Window</span>
              <span className="text-xl font-bold text-emerald-400 mt-1 block">{candidates.activeCandidates}</span>
            </div>
            <div className="p-3.5 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[11px] font-semibold text-slate-500 block uppercase">Avg Attempts / Candidate</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <h3 className="text-xs font-bold text-slate-900 mb-3">Overall Ability (θ) Distribution</h3>
              <div className="space-y-2">
            ...
            {/* SE Distribution */}
            <div className="p-4 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <h3 className="text-xs font-bold text-slate-900 mb-3">Standard Error (SE) Precision Profile</h3>
              <div className="space-y-2">
          ...
          <div className="p-4 rounded-xl bg-white border border-slate-200/80 shadow-xs">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {candidates.weakestTopics.map((t) => (
                <div key={t.topicId} className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-mono text-slate-300 truncate max-w-[180px]">{t.topicName}</span>
                    <Badge variant="destructive" className="text-[10px] font-mono bg-rose-50 text-rose-700 border-rose-200 font-bold">
          ...
          <div className="p-4 rounded-xl bg-white border border-slate-200/80 shadow-xs">
            <h3 className="text-xs font-bold text-slate-900 mb-3">Candidate Longitudinal Profiles</h3>
                    ...
                    <tr key={c.userId} className="hover:bg-slate-50/80">
                      ...
                      <td className="py-2.5 px-3 text-right">
                        <button
                          ...
                          className="px-2.5 py-1 rounded bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 text-xs font-medium transition"
                        >
                          Inspect
                        </button>
                      </td>
          ...
          {selectedCandidate && (
            <div className="fixed inset-0 z-50 bg-slate-900/50 flex items-center justify-center p-4">
              <div className="bg-white border border-slate-200/80 rounded-xl shadow-xs p-5 max-w-2xl w-full max-h-[85vh] overflow-y-auto space-y-4">
                <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                  <div>
                    <h3 className="text-sm font-bold text-slate-900">Candidate Adaptive Longitudinal Profile</h3>
                    <p className="text-xs font-mono text-slate-400">{selectedCandidate.userId}</p>
                  </div>
                  <button onClick={() => setSelectedCandidate(null)} className="text-slate-400 hover:text-slate-900">✕</button>
                </div>

                <div className="grid grid-cols-3 gap-3">
                  <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                    <span className="text-[10px] text-slate-500 block">Overall Ability (θ)</span>
                    <span className="text-base font-bold text-blue-400">{selectedCandidate.overallTheta}</span>
                  </div>
                  <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                    <span className="text-[10px] text-slate-500 block">Standard Error</span>
                  ...
                  <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                    ...
                    <span className="text-base font-bold text-slate-900">{selectedCandidate.totalQuestionsAnswered}</span>
                  </div>
                </div>

                <div className="space-y-2">
                  <h4 className="text-xs font-bold text-slate-900">Topic Mastery Vector</h4>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    {Object.entries(selectedCandidate.topicMasteries).map(([tid, data]) => (
                      <div key={tid} className="p-2 rounded bg-slate-50 border border-slate-200 flex justify-between">
                ...
                <div className="space-y-2">
                  <h4 className="text-xs font-bold text-slate-900">Recent Attempt History</h4>
                  <div className="space-y-1.5 text-xs">
                    {selectedCandidate.attemptHistory.map((h) => (
                      <div key={h.attemptId} className="p-2 rounded bg-slate-50 border border-slate-200 flex items-center justify-between">
                        <div>
                          <span className="font-bold text-slate-900 block">{h.examTitle}</span>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
            <div className="p-3 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[10px] font-semibold text-slate-500 block uppercase">Total Bank</span>
              <span className="text-lg font-bold text-slate-900">{items.totalItems}</span>
            </div>
            <div className="p-3 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[10px] font-semibold text-slate-500 block uppercase">Calibrated</span>
              <span className="text-lg font-bold text-emerald-400">{items.calibratedItems}</span>
            </div>
            <div className="p-3 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[10px] font-semibold text-slate-500 block uppercase">Provisional</span>
              <span className="text-lg font-bold text-blue-400">{items.provisionalItems}</span>
            </div>
            <div className="p-3 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[10px] font-semibold text-slate-500 block uppercase">Uncalibrated</span>
              <span className="text-lg font-bold text-slate-400">{items.uncalibratedItems}</span>
            </div>
            <div className="p-3 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[10px] font-semibold text-slate-500 block uppercase">Flagged</span>
              <span className="text-lg font-bold text-rose-400">{items.flaggedItems}</span>
            </div>
            <div className="p-3 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[10px] font-semibold text-slate-500 block uppercase">Deprecated</span>
              <span className="text-lg font-bold text-slate-500">{items.deprecatedItems}</span>
            </div>
            <div className="p-3 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              <span className="text-[10px] font-semibold text-slate-500 block uppercase">Quality Alerts</span>
              <span className="text-lg font-bold text-amber-400">{items.itemsWithAlertsCount}</span>
            </div>
          </div>

          {/* Quality Filter Bar */}
          <div className="flex flex-wrap items-center justify-between gap-3 p-3 rounded-lg bg-white border border-slate-200/80 shadow-xs">
            <div className="flex items-center gap-2">
              <span className="text-xs text-slate-500 font-semibold">Filter Flag:</span>
              <select
                value={itemQualityFilter}
                onChange={(e) => setItemQualityFilter(e.target.value as ItemQualityFlag | "ALL")}
                className="bg-slate-50 border border-slate-200 rounded-md px-2.5 py-1 text-xs text-slate-900"
              >
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="bg-slate-50 border border-slate-200 rounded-md px-2.5 py-1 text-xs text-slate-900 w-56"
              />
              <button
                onClick={loadAnalytics}
          ...
          {/* Items Table */}
          <div className="p-4 rounded-xl bg-white border border-slate-200/80 shadow-xs overflow-x-auto">
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-200 text-slate-500 font-medium">
              ...
              <tbody className="divide-y divide-slate-100">
                {items.items.map((row) => (
                  <tr key={row.questionId} className="hover:bg-slate-50/80">
                        ...
                        {row.qualityFlags.length === 0 ? (
                          ...
                          row.qualityFlags.map((f) => (
                            <Badge
                              key={f}
                              variant="destructive"
                              className="text-[9px] font-mono px-1 py-0 bg-amber-50 text-amber-800 border-amber-200"
                            >
                              {f}
                            </Badge>
                          ))
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              ...
              <div className="space-y-2">
                {cat.strategyBreakdown.map((s) => (
                  <div key={s.strategy} className="p-2 rounded-lg bg-slate-50 border border-slate-200 flex justify-between text-xs">
              ...
              <div className="mt-4 pt-3 border-t border-slate-200 flex justify-between text-xs">
                <span className="text-slate-400">Avg Fisher Information: <strong className="text-slate-900">{cat.avgSelectedInformation}</strong></span>
                <span className="text-slate-400">Median Info: <strong className="text-slate-900">{cat.medianSelectedInformation}</strong></span>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                  <span className="text-[10px] text-slate-500 block">Convergence Rate</span>
                  <span className="text-lg font-bold text-emerald-400">{ability.convergenceRate}%</span>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                  <span className="text-[10px] text-slate-500 block">Avg Step Δθ</span>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                  <span className="text-[10px] text-slate-500 block">Early Stopping Rate</span>
                  <span className="text-lg font-bold text-emerald-400">{stopping.earlyStoppingRate}%</span>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                  <span className="text-[10px] text-slate-500 block">Target SE Met Rate</span>
              ...
              <div className="space-y-1.5 text-xs">
                {stopping.continuationGatesEncountered.map((g) => (
                  <div key={g.gate} className="p-2 rounded bg-slate-50 border border-slate-200 flex justify-between font-mono">
            ...
            <div className="p-4 rounded-xl bg-white border border-slate-200/80 shadow-xs">
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                  <span className="text-[10px] text-slate-500 block">Cold Start Rate</span>
                  <span className="text-lg font-bold text-slate-300">{personalization.coldStartRate}%</span>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                  <span className="text-[10px] text-slate-500 block">Avg Influence Score</span>
            ...
            <table className="w-full text-left text-xs border-collapse">
              ...
              <tbody className="divide-y divide-slate-100">
                {algorithmComparison.algorithms.map((a) => (
                  <tr key={a.versionSlug} className="hover:bg-slate-50/80">
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 rounded-xl bg-white border border-slate-200/80 shadow-xs space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {health.kpis.map((k) => (
                <div key={k.name} className="p-3 rounded-lg bg-slate-50 border border-slate-200">
                  <div className="flex items-center justify-between">
          ...
          <div className="p-4 rounded-xl bg-white border border-slate-200/80 shadow-xs space-y-4">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-1.5 text-xs text-slate-900 w-80 font-mono"
              />
              <button
                onClick={handleInspectAttempt}
                disabled={diagnosticsLoading}
                className="px-4 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition flex items-center gap-1.5"
              >
              ...
              <div className="space-y-4 pt-3 border-t border-slate-200">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="p-2.5 rounded bg-slate-50 border border-slate-200">
                    ...
                    <span className="text-xs font-mono font-bold text-slate-900 truncate block">{attemptDiagnostics.attemptId}</span>
                  </div>
                  <div className="p-2.5 rounded bg-slate-50 border border-slate-200">
                    <span className="text-[10px] text-slate-500 block">Final Ability (θ) / SE</span>
                  ...
                  <div className="p-2.5 rounded bg-slate-50 border border-slate-200">
                    <span className="text-[10px] text-slate-500 block">Stopping Reason</span>
                  ...
                  <table className="w-full text-left text-xs border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 font-mono">
                      {attemptDiagnostics.steps.map((s) => (
                        <tr key={s.stepNumber} className="hover:bg-slate-50/80">
                          <td className="py-2 px-2 font-bold text-slate-900">{s.stepNumber}</td>
