...
export function ContentStudioView({
  ...
  return (
    <div className="flex h-[calc(100vh-80px)] flex-col overflow-hidden bg-slate-50 dark:bg-slate-950">
      {/* Top Navigation Bar */}
      <div className="flex items-center justify-between border-b border-slate-200 bg-white px-6 py-3 dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-black text-slate-900 text-base dark:text-slate-100">Admin Content Studio</h1>
          ...
          <div className="flex rounded-xl border border-slate-200 bg-slate-100 p-0.5 dark:border-slate-800 dark:bg-slate-800">
            <button
              ...
              className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 font-bold text-xs transition ${
                activeTab === "EXPLORER" ? "bg-white text-indigo-600 shadow-2xs dark:bg-slate-900 dark:text-indigo-400" : "text-slate-600 dark:text-slate-400"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 font-bold text-xs transition ${
                activeTab === "COVERAGE" ? "bg-white text-indigo-600 shadow-2xs dark:bg-slate-900 dark:text-indigo-400" : "text-slate-600 dark:text-slate-400"
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 font-bold text-xs transition ${
                activeTab === "QUEUE" ? "bg-white text-indigo-600 shadow-2xs dark:bg-slate-900 dark:text-indigo-400" : "text-slate-600 dark:text-slate-400"
              }`}
            >
      ...
      {activeTab === "QUEUE" ? (
        ...
        <div className="grid flex-1 grid-cols-12 overflow-hidden">
          ...
          <div className="col-span-6 flex h-full flex-col border-r border-slate-200 bg-white overflow-hidden dark:border-slate-800 dark:bg-slate-900">
            {selectedUnitId ? (
              <div className="flex-1 overflow-y-auto">
                <div className="border-b border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-800/40">
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="font-semibold text-[10px] text-indigo-600 uppercase tracking-wider">Learning Unit</span>
                      <h2 className="font-bold text-slate-900 text-lg dark:text-slate-100">{unitDetail?.unit?.title || "Unit Workspace"}</h2>
                    </div>
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => setShowAIModal(true)}
                        className="flex items-center gap-1.5 rounded-lg bg-gradient-to-r from-violet-600 to-indigo-600 px-3 py-1.5 font-bold text-white text-xs hover:from-violet-700 hover:to-indigo-700 shadow-2xs"
                      >
                      ...
                      <button
                        ...
                        disabled={isSaving}
                        className="flex items-center gap-1.5 rounded-lg bg-indigo-600 px-3 py-1.5 font-bold text-white text-xs hover:bg-indigo-700 shadow-2xs"
                      >
                ...
                {currentSpec ? (
                  ...
                  <div className="p-8 text-center text-slate-400">
                    ...
                    <button
                      ...
                      className="mt-3 inline-flex items-center gap-1.5 rounded-lg bg-indigo-600 px-4 py-2 font-bold text-xs text-white"
                    >
              ...
              <div className="flex flex-1 items-center justify-center p-8 text-center text-slate-400">
                <div>
                  ...
                  <h3 className="mt-3 font-bold text-slate-700 text-sm dark:text-slate-300">Select a Learning Unit</h3>
                  <p className="mt-1 text-xs">Navigate the academic taxonomy on the left to author or review content.</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="col-span-3 flex h-full flex-col overflow-y-auto bg-slate-50 dark:bg-slate-900">
            <ValidationPanel
