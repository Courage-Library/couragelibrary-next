...
interface Props {
  params: Promise<{ slug: string; cycleYear: string; moduleSlug: string }>;
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug, cycleYear, moduleSlug } = await params;
  const yearNum = parseInt(cycleYear, 10);
  const moduleKey = ExamModuleRegistry.getModuleKeyFromSlug(moduleSlug);

  if (isNaN(yearNum) || !moduleKey) {
  ...
  return constructMetadata({
    title: `${modDef.displayName} (${yearNum}) — ${exam.title}`,
    description: modDef.purpose,
    canonicalUrl: `/exams/${slug}/cycle/${cycleYear}/${moduleSlug}`,
  });
}

export default async function ExamCycleModuleReaderPage({ params }: Props) {
  const { slug, cycleYear, moduleSlug } = await params;
  const yearNum = parseInt(cycleYear, 10);
  const moduleKey = ExamModuleRegistry.getModuleKeyFromSlug(moduleSlug);
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        "@type": "ListItem",
        position: 4,
        name: `${publishedModule.displayName} (${yearNum})`,
        item: `https://couragelibrary.com/exams/${data.exam.slug}/cycle/${yearNum}/${moduleSlug}`,
      },
    ],
  };
